...
class ArmEnv(object):
    ...
    def step(self, action):
        ...
        a2xy_ =np.array([np.cos(a1r+a2r), np.sin(a1r+a2r)]) * a1l + a1xy
        finger = np.array([np.cos(a1r + a2r +a3r), np.sin(a1r + a2r+a3r)]) * a2l + a1xy_  # a2 end (x2, y2)
        # normalize features
    ...
    def reset(self):
        ...
        (a1r, a2r,a3r) = self.arm_info['r']  # radian, angle
        a1xy = np.array([200., 200.])  # a1 start (x0, y0)
        a1xy_ = np.array([np.cos(a1r), np.sin(a1r)]) * a1l + a1xy  # a1 end and a2
        a2xy_ = np.array([np.cos(a1r+a2r), np.sin(a1r+a2r)]) * a1l + a1xy
        start (x1, y1)
        finger = np.array([np.cos(a1r + a2r+a3r), np.sin(a1r + a2r+a3r)]) * a2l + a1xy_  # a2 end (x2, y2)
        # normalize features
